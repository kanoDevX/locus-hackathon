...
/// <summary>
/// Account editing (§ UX request: "click to edit your account"). Deliberately separate from
/// RegisterCommand/LoginCommand — those issue tokens, this only ever updates the two fields a
/// user plausibly wants to change about themselves post-signup. Role is never editable here
/// (that stays an admin-only concern the app doesn't expose a surface for yet).
/// </summary>
...
/// <summary>Changing the password revokes every active refresh token (same "log out everywhere"
/// behavior as LogoutAllCommand) — a credential change should never leave an older session
/// silently valid on another device. The frontend tells the user they'll need to sign back in.</summary>
